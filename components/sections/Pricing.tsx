...
export default function Pricing() {
  ...
  return (
    <section id="pricing" className="py-24 px-6 relative overflow-hidden bg-[#0E0E10]">
      {/* BG accents */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#8B1A1A]/10 rounded-full blur-[150px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#C0392B]/8 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div ref={ref} className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="reveal reveal-delay-1 flex justify-center mb-12">
          <div
            ...
            style={{ background: "linear-gradient(90deg, rgba(245,158,11,0.15), rgba(239,68,68,0.1))", border: "1px solid rgba(245,158,11,0.3)" }}
          >
            <span className="text-amber-400 animate-pulse">🔥</span>
            <span className="text-amber-300">Limited Time: Up to 65% OFF — Ends Soon</span>
            <span className="text-amber-400 animate-pulse">🔥</span>
          </div>
        </div>
        ...
        {/* Guarantee */}
        <div className="reveal mt-12 flex flex-wrap justify-center gap-6 text-sm text-[#8C8C9C]">
  ...
  return (
    <div
      className={`reveal reveal-delay-${index + 1} relative flex flex-col rounded-2xl overflow-hidden transition-all duration-300 hover:scale-[1.02] ${
        plan.popular
          ? "ring-1 ring-[#C0392B]/70 animate-[glowPulse_3s_ease-in-out_infinite] scale-[1.02] lg:scale-[1.05]"
          : "glass-card hover:ring-1 hover:ring-[#C0392B]/30"
      }`}
      style={
        plan.popular
          ? {
              background: "linear-gradient(160deg, rgba(192,57,43,0.18), rgba(232,160,144,0.08))",
              boxShadow: "0 0 40px rgba(192,57,43,0.25), 0 0 80px rgba(192,57,43,0.1)",
            }
          : {}
      }
    >
      ...
      <div className={`p-7 flex flex-col h-full ${plan.popular ? "pt-14" : ""}`}>
        {/* Plan label */}
        <div className="mb-4">
          <span className="text-[#8C8C9C] text-xs font-bold uppercase tracking-widest">{plan.label}</span>
          {savings > 0 && (
        ...
        <div className="mb-6">
          <div className="flex items-baseline gap-2">
            <span className={`font-display font-black text-5xl ${plan.popular ? "text-[#C0392B]" : "text-[#EDE9E0]"}`}>
              ${plan.price}
            </span>
            <span className="text-[#8C8C9C] text-sm">total</span>
          </div>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[#8C8C9C] text-sm line-through">${plan.originalPrice}</span>
          ...
          {plan.features.map((feat) => (
            <li key={feat} className="flex items-start gap-2.5 text-sm">
              <span
                ...
                style={{ background: plan.popular ? "rgba(192,57,43,0.3)" : "rgba(255,255,255,0.1)" }}
              >
                ✓
              </span>
              <span className={plan.popular ? "text-[#EDE9E0]" : "text-[#8C8C9C]"}>{feat}</span>
            </li>
          ))}
        </ul>
