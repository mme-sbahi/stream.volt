        ...
        <ul className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="px-4 py-2 rounded-lg text-[#8C8C9C] hover:text-[#EDE9E0] hover:bg-white/5 transition-all duration-200 text-sm font-medium"
              >
                {link.label}
              </a>
            </li>
        ...
        <div className="px-6 py-4 flex flex-col gap-1">
          {NAV_LINKS.map((link) => (
            <a
              ...
              href={link.href}
              onClick={closeMenu}
              className="py-2.5 px-3 rounded-lg text-[#8C8C9C] hover:text-[#EDE9E0] hover:bg-white/5 transition-all"
            >
              {link.label}
            </a>
          ))}
